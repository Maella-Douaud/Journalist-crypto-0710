...
puts "Il y a " + handle.count.to_s + " journalistes"
...
puts handle.select{|handle| handle.length == 6}

handle_6C = handle.select{|handle| handle.length == 6}
puts "Il y a " + handle_2C.count.to_s + " handle avec 6 caractères."



#Combien commencent par une majuscule (première lettre juste après le @) ?
puts handle.select{ |i| i[1] == i[1].capitalize }



#Trie la liste de handle par taille des handle (les plus petits en premiers, les plus grands après)
puts handle.sort_by { |word| word.length }



#Quelle est la position dans l'array de la personne @epenser ?
def handle.index('@epenser')

puts "@epenser se trouve à la position #{andle.index('@epenser')}" 
...
#TEST
puts handle.select{|handle| handle.length == 2}

handle_2C = handle.select{|handle| handle.length == 2}
puts "Il y a " + handle_2C.count.to_s + " handle avec 2 caractères."

puts handle.select{|handle| handle.length == 3}

handle_3C = handle.select{|handle| handle.length == 3}
puts "Il y a " + handle_3C.count.to_s + " handle avec 3 caractères."
#TEST